/*
 * Samba-VirusFilter VFS modules: Utility functions
 *
...
int svf_shell_set_conn_env(svf_env_struct *env_h, connection_struct *conn)
{
	...
	svf_env_set(env_h, SVF_ENV_PREFIX "SERVER_IP", server_addr_p);
	svf_env_set(env_h, SVF_ENV_PREFIX "SERVER_NAME", myhostname());
	svf_env_set(env_h, SVF_ENV_PREFIX "SERVER_NETBIOS_NAME", local_machine_name);
	slprintf(pidstr,sizeof(pidstr)-1, "%ld", (long)getpid());
	svf_env_set(env_h, SVF_ENV_PREFIX "SERVER_PID", pidstr);

	svf_env_set(env_h, SVF_ENV_PREFIX "SERVICE_NAME", lp_const_servicename(snum));
	svf_env_set(env_h, SVF_ENV_PREFIX "SERVICE_PATH", conn->connectpath);

	client_addr_p = conn_client_addr(conn);
	if (strncmp("::ffff:", client_addr_p, 7) == 0) {
		client_addr_p += 7;
	}
	svf_env_set(env_h, SVF_ENV_PREFIX "CLIENT_IP", client_addr_p);
	svf_env_set(env_h, SVF_ENV_PREFIX "CLIENT_NAME", conn_client_name(conn));
	svf_env_set(env_h, SVF_ENV_PREFIX "CLIENT_NETBIOS_NAME", get_remote_machine_name());

	svf_env_set(env_h, SVF_ENV_PREFIX "USER_NAME", get_current_username());
	svf_env_set(env_h, SVF_ENV_PREFIX "USER_DOMAIN", current_user_info.domain);
